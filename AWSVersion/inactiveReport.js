...
module.exports = async function generateInactiveReport(responseTeams, week){
    ...
    let listOfTeamsWithGame = await getTeamsWithGame(week);
        ...
        const getInactiveStartingPlayers = playersOnFantasyRosters.filter(player => player.position != "Bench" && player.position != "IR" && player.injuryStatus != "ACTIVE" && player.injuryStatus != "INJURY_RESERVE" && player.injuryStatus != undefined);
